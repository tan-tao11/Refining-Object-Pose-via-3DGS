...
@torch.no_grad()
def _render_gs_features(splats, viewmat, K, W, H):
    """Render GS feature map using gsplat's rasterization kernel.

    Args:
        splats: dict with keys means, quats, scales, opacities, gs_features.
        viewmat: (4, 4) world-to-camera matrix.
        K: (3, 3) camera intrinsic matrix.
        W, H: render resolution.

    Returns:
        (H, W, C) rendered feature map.
    """
    means = splats['means']                        # (N, 3)
    quats = splats['quats']                        # (N, 4)
    scales = torch.exp(splats['scales'])           # (N, 3)
    opacities = torch.sigmoid(splats['opacities']) # (N,)
    gs_features = splats['gs_features']            # (N, C)
...
def render_features_for_batch(data, renderer_cache, device):
    ...
    for i in range(batch_size):
        ...
        R = data['initial_R'][i]   # (3, 3) world-to-camera rotation
        t = data['initial_t'][i]   # (3,)   world-to-camera translation

        # Construct w2c viewmat directly (initial_R/t are already w2c)
        ...
        feat = _render_gs_features(splats, viewmat, K, W, H)
...
def train_refine_worker(gpu_id: int, config: OmegaConf):
    ...
    # ---- Validation Dataloader (rank 0 only) ----
    val_loader = None
    if gpu_id == 0 and config.dataset.get('val_anno_file'):
        val_dataset = OnePoseRefineDataset(
    ...
    optimizer = torch.optim.Adam([
        {'params': backbone_params, 'lr': config.train.learning_rate * 0.1},
        {'params': new_params, 'lr': config.train.learning_rate},
    ])
    ...
    for epoch in range(max_epochs):
        ...
        for _, data in enumerate(data_loader):
            ...
            if gpu_id == 0:
                ...
                val_itr = config.train.get('val_itr', config.train.save_itr)
                if val_loader is not None and step % val_itr == 0:
                    val_metrics = validate_refine(
                        model.module, val_loader, renderer_cache, device,
                    )
                    if val_metrics:
                        writer.add_scalar('Val/R_err_ref_mean', val_metrics['R_err_ref_mean'], step)
                        writer.add_scalar('Val/R_err_ref_med', val_metrics['R_err_ref_med'], step)
                        writer.add_scalar('Val/t_err_ref_mean', val_metrics['t_err_ref_mean'], step)
                        writer.add_scalar('Val/t_err_ref_med', val_metrics['t_err_ref_med'], step)
                        writer.add_scalar('Val/R_improve', val_metrics['R_improve'], step)
                        writer.add_scalar('Val/t_improve', val_metrics['t_improve'], step)
                        logger.info(
                            f"[Val step={step}] "
                            f"R_err: {val_metrics['R_err_init_mean']:.2f}°→{val_metrics['R_err_ref_mean']:.2f}° "
                            f"(Δ{val_metrics['R_improve']:+.2f}°)  "
                            f"t_err: {val_metrics['t_err_init_mean']*100:.1f}cm→{val_metrics['t_err_ref_mean']*100:.1f}cm "
                            f"(Δ{val_metrics['t_improve']*100:+.1f}cm)"
                        )
                    model.train()
...
@torch.no_grad()
def validate_refine(model, val_loader, renderer_cache, device, max_samples=200):
    """Run validation and return pose error metrics."""
    model.eval()
    ...
    for batch_idx, data in enumerate(val_loader):
        ...
        for i in range(data['R_gt'].shape[0]):
            R_gt = data['R_gt'][i]
            t_gt = data['t_gt'][i]

            R_errs_init.append(_geodesic_angle(data['initial_R'][i], R_gt))
            t_errs_init.append((data['initial_t'][i] - t_gt).norm().item())

            R_errs_ref.append(_geodesic_angle(data['R_refined'][i], R_gt))
            t_errs_ref.append((data['t_refined'][i] - t_gt).norm().item())
    ...
    R_init_arr = np.array(R_errs_init)
    t_init_arr = np.array(t_errs_init)
    R_ref_arr = np.array(R_errs_ref)
    t_ref_arr = np.array(t_errs_ref)

    return {
        'R_err_init_mean': R_init_arr.mean(),
        'R_err_init_med': np.median(R_init_arr),
        't_err_init_mean': t_init_arr.mean(),
        't_err_init_med': np.median(t_init_arr),
        'R_err_ref_mean': R_ref_arr.mean(),
        'R_err_ref_med': np.median(R_ref_arr),
        't_err_ref_mean': t_ref_arr.mean(),
        't_err_ref_med': np.median(t_ref_arr),
        'R_improve': R_init_arr.mean() - R_ref_arr.mean(),
        't_improve': t_init_arr.mean() - t_ref_arr.mean(),
    }
...
def _render_single_gs_feature(splats, R_init, t_init, K, img_resize, device):
    """Render a GS feature map for one sample given the initial pose (w2c)."""
    viewmat = torch.eye(4, device=device)
    viewmat[:3, :3] = R_init
    viewmat[:3, 3] = t_init
    ...
    feat = _render_gs_features(splats, viewmat, K_half, W, H)
