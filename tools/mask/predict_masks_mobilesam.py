...
def main(image_folder, text_prompt, output_dir_mask, BOX_THRESHOLD=0.25, TEXT_THRESHOLD=0.25, NMS_THRESHOLD=0.5):
    DEVICE = 'cuda'
    ...
    grounding_dino_model = Model(model_config_path=groundingdino_config, model_checkpoint_path=groundingdino_ckpt_path)

    # Building MobileSAM predictor
    MOBILE_SAM_CHECKPOINT_PATH = 'tools/mask/checkpoints/mobile_sam.pt'
    ...
    for image_file in tqdm(images):
        ...
        cv2.imwrite(os.path.join(output_dir_mask, image_file), binary_mask)
...
def predict_masks_mobilesam(image_folder,  text_prompt, output_dir_mask):
    # files = os.listdir(image_folder)
    # images = [file for file in files if file.endswith('.png')]
