...
class ResNetFPN_8_2(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        # Config
        block = block_type['BasicBlock']
        initial_dim = 128
        block_dims = [128, 196, 256]
        self.block_dims = block_dims
        self.output_layers = [3, 1]
...
def resnet18_c3(pretrained=False, in_chans=1, config=ResNet18C3.default_cfg, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(checkpoint)
    return model
def build_backbone(network, ckpt, device='cuda:0'):
    if network == 'ResNetFPN_8_2':
        net = ResNetFPN_8_2(config=None)
        
        ckpt = torch.load(ckpt, 'cpu')['state_dict']
