...
def has_existing_gs_model(output_path):
    ckpt_dir = os.path.join(output_path, "ckpts")
    return bool(glob.glob(os.path.join(ckpt_dir, "*.pt")))
...
if __name__ == "__main__":
    ...
    for obj_folder in obj_folders:
        ...
        if has_existing_gs_model(output_path):
            print(f"Skip {obj_folder}: existing GS checkpoint found in {output_path}/ckpts")
            continue
