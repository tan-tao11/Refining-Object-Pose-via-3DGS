...
def pad_keypoints3d_top_n(keypoints, n_target_kpts):
    """ Pad or truncate original 3D keypoints to a fixed size."""
...
def pad_keypoints3d_according_to_assignmatrix(keypoints, n_target_kpts, assignmatrix):
    ...
    if n_pad < 0:
        ...
        # Aim to keep all matches in the assignmatrix
        # Get matched 3D points coordinates:
        ...
        # Fix bug: If the number of matched 3D points exceeds the target quantity, directly truncate
        if num_of_pad < 0:
            keypoints_padded = matched_3D_points[:n_target_kpts]
            assignmatrix = assignmatrix[:, :n_target_kpts] # Adjust indices again
            index_shuffled = assignmatrix[1, :].clone()
            assignmatrix[1, :] = torch.arange(n_target_kpts)
            
            # assignmatrix[1, :] = torch.arange(n_target_kpts)  # Adjust indices again
            return keypoints_padded, assignmatrix, index_shuffled  # Return directly to avoid subsequent errors
...
def pad_features3d_top_n(descriptors, scores, n_target_shape):
    """ Pad or truncate original 3D features (descriptors and scores) to a fixed size."""
...
def get_K_crop_resize(box, K_orig, resize_shape):
    ...
    """
    center = np.array([(box[0] + box[2]) / 2.0, (box[1] + box[3]) / 2.0])
    scale = np.array([box[2] - box[0], box[3] - box[1]])  # width, height
