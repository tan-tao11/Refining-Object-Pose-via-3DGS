...
class OnePoseDataset(Dataset):
    def __init__(self,
        ...
        match_type='softmax'):
        '''
        Initialize the OnePose dataset class.

        Args:
            cfg (module): OnePose config module
            device (torch.device): The device for data loading
            anno_file (str): The path to the annotation file
            pad (bool): Whether to pad the 2D image feature map. Defaults to True.
            img_pad (bool): Whether to pad the 2D image. Defaults to False.
            img_resize (int): The size for resizing the 2D image. Defaults to 512.
            coarse_scale (float): The scale factor for downsampling the 2D image feature map. Defaults to 1/8.
            df (int): The downsampling factor for downsampling the 2D image feature map. Defaults to 8.
            shape3d (int): The number of 3D points. Defaults to 10000.
            percent (float): The percentage of data to use. Defaults to 1.0.
            split (str): The dataset split. Defaults to "train".
            load_pose_gt (bool): Whether to load the ground truth pose. Defaults to False.
            load_3d_coarse_feature (bool): Whether to load the coarse 3D feature. Defaults to False.
            image_warp_adapt (bool): Whether to adapt the image warping. Defaults to False.
            augmentor (module): The data augmentor module. Defaults to None.
            match_type (str): The type of matching. Defaults to 'softmax'.
        '''
    ...
    def read_anno(self, img_id, image_warp_adapt=False):
        ...
        if self.split == "train":
            ...
            # print(keypoints2d_coarse.shape, assign_matrix.shape)
            if keypoints2d_coarse.shape[0] <= 0:
                print(anno2d_coarse_file)
        else:
            assign_matrix = None
        ...
        if self.split == "train":
            ...
            # print(mkpts_proj_rounded.shape)
